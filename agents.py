...
class MinimaxPruneAgent(MinimaxAgent):
    ...
    def minimax(self, state):
        ...
        return self.alphabeta(state, -math.inf, math.inf, maxPlayer)
    ...
    def alphabeta(self, state, alpha, beta, maxPlayer):
        ...
        if maxPlayer:
            ...
            for move, succ_state in state.successors():
                val = max(val, self.alphabeta(succ_state, alpha, beta, False))
                if val >= beta:
                    break
                alpha = max(alpha,val)
            return val
            ...
            val = math.inf
            for move, succ_state in state.successors():
                val = min(val, self.alphabeta(succ_state, alpha, beta, True))
                if val <= alpha:
                    break
                beta = min(beta,val)
            return val
